...
def test_returns_job_when_match(mocker):
...
def test_returns_none_when_only_comment_matches(mocker):
    job = mocker.MagicMock()
    job.comment = "backup-job"
...
def test_get_search_string_no_slash(mocker, autocomplete):
...
def test_get_search_string_with_slash(mocker, autocomplete):
    state = mocker.MagicMock()
...
def test_get_search_string_multiple_words(mocker, autocomplete):
    state = mocker.MagicMock()
